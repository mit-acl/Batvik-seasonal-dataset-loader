...
class OrthoImageLoader:
    ...
    def getMeshgrid(self):
        if (self.row_mesh is None or self.col_mesh is None):
            ...
            self.col_mesh,self.row_mesh = np.meshgrid(cols_vect,rows_vect, indexing='ij', sparse=True)
    ...
    def interpolatePoint(self, colVal, rowVal):
        if (self.interp is None):
            #rowMesh, colMesh= self.getMeshgrid()

            cols_vect = np.linspace(self.left,self.right,num=self.im_w_px)
            rows_vect = np.linspace(self.top,self.bottom,num=self.im_h_px)

            self.interp = RegularGridInterpolator((rows_vect,cols_vect), self.imagedata)

        return np.squeeze(self.interp((rowVal,colVal)))
